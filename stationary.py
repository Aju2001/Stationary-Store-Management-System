...
my_cursor.execute("create table if not exists stationary(item_name VARCHAR(20) not null unique, item_price INTEGER(10), item_quantity INTEGER(10), item_category VARCHAR(20), item_discount float(3),grand_total float(5))")
##############################################################################################
root = Tk() 
...
def showdatabase():
    ...
    mytext.insert(END," Item_Name \t\tItem_Price \t\tItem_Quantity \t\tItem_Category \t\tItem_Discount \t\tTotal Amount \n")
    mytext.insert(END," ------------ \t\t----------- \t\t-------------- \t\t--------------- \t\t--------------- \t\t---------------\n")
    for row in mytext1:
        i += 1
        mytext.insert(END,"       {0} \t\t     {1} \t\t         {2} \t\t   {3} \t\t          {4} \t\t          {5}\n".format(row[0],row[1],row[2],row[3],row[4],(float(row[1])-float(row[4]))*float(row[2])))
